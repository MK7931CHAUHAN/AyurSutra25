...
const fetchPatients = async () => {
  try {
    setLoading(true);
    // Remove pagination parameters to get all patients
    ...
    if (response.data.success) {
      const transformedPatients = response.data.patients.map(patient => ({
        ...
        createdBy: patient.createdBy, // Add createdBy field
        rawData: patient
      }));

      setPatients(transformedPatients);
      setFilteredPatients(transformedPatients); // Set filtered patients too
    ...
    setTotalPages(Math.ceil(mockPatients.length / ITEMS_PER_PAGE));
...
// Remove useEffect dependency on currentPage
useEffect(() => {
  fetchPatients();
  fetchStats();
        ...
        <table className="min-w-full border-collapse">
          <thead>
            <tr className="bg-linear-to-r from-gray-50 to-gray-100 border-b border-gray-200">
              ...
              <th className="px6 py-4 text-left text-sm font-semibold text-gray-700 uppercase tracking-wider">
                Contact Info
              </th>
              <th className="px6 py-4 text-left text-sm font-semibold text-gray-700 uppercase tracking-wider">
                Demographics
          ...
          <tbody className="divide-y divide-gray-100">
            {displayedPatients.map((patient, index) => {
              ...
              // If patient has photo, use it
              const hasPhoto = patient.photo && patient.photo.trim() !== '';
                        ...
                        <img
                          ...
                          onError={(e) => {
                            ...
                            e.target.parentNode.innerHTML = `
                              <div class="w-12 h-12 rounded-xl flex items-center justify-center font-bold text-lg shadow-sm ${avatarColors[avatarColor]}">
                                ${patient.initials}
                              </div>
                            `;
                          }}
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          {patient.createdBy && (
                            <span className="text-xs text-gray-500 truncate max-w-[120px]">
                              By: {patient.createdBy.name || patient.createdBy.email}
                            </span>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="space-y-1">
                      ...
                      {patient.emergencyContact && patient.emergencyContact.phone && (
                        <div className="flex items-center gap-2 mt-2">
                          ...
                          <span className="text-xs text-gray-500">Emergency: {patient.emergencyContact.phone}</span>
                        </div>
                      )}
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-5 whitespace-nowrap">
                    <div className="flex flex-col gap-2">
                      <button
                        ...
                        className="flex items-center justify-center gap-2 px-4 py-2 bg-teal-50 text-teal-700 rounded-lg hover:bg-teal-100 hover:text-teal-800 transition-all border border-teal-100"
                      >
                      ...
                      <button
                        ...
                        className="flex items-center justify-center gap-2 px-4 py-2 bg-blue-50 text-blue-700 rounded-lg hover:bg-blue-100 hover:text-blue-800 transition-all border border-blue-100 text-sm"
                      >
                        Full Profile
                      </button>
                    </div>
