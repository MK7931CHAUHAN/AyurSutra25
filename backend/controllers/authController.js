...
/* ======================================================
   HELPER FUNCTIONS
====================================================== */
const validateGoogleEmail = async (email) => {
  try {
    // Simple regex validation for Gmail
...
/* ======================================================
   REGISTER USER with Google Email Verification
====================================================== */
exports.register = async (req, res) => {
  try {
    const { name, email, phone, password, role = "patient" } = req.body;
    ...
    const emailValidation = await validateGoogleEmail(email);
    ...
    // Generate temporary token for unverified users
    const token = jwt.sign(
      { 
        id: user._id, 
        role: user.role,
...
exports.login = async (req, res) => {
  try {
    ...
    // Generate JWT token
    const token = jwt.sign(
      { id: user._id, role: user.role, emailVerified: user.isEmailVerified },
      process.env.JWT_SECRET,
      { expiresIn: process.env.JWT_EXPIRE || "7d" }
    );
